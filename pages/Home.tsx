...
const Home: React.FC = () => {
          ...
          <div className="overflow-y-auto">
            ...
            <div className="p-8 md:p-16 -mt-20 relative z-10">
               <div className="flex items-center gap-4 mb-8">
                  <span className="px-4 py-1.5 bg-brand-primary text-white text-[10px] font-black uppercase tracking-widest rounded-md">
                    {item.category}
                  </span>
                  <span className="text-brand-accent font-black text-[11px] tracking-widest">{item.date}</span>
               </div>
               <h2 className="text-3xl md:text-5xl font-serif font-black text-brand-ink leading-tight mb-8">
                  {item.title}
               </h2>
               <div className="prose prose-stone max-w-none text-brand-sub text-[16px] leading-loose font-light whitespace-pre-line">
                  {item.content || item.summary}
               </div>
               <div className="mt-12 pt-8 border-t border-brand-border flex flex-col md:flex-row gap-6 items-center justify-between">
                  <Link to="/booking" onClick={onClose} className="w-full md:w-auto px-10 py-4 bg-brand-primary text-white rounded-xl font-black text-[13px] uppercase tracking-[0.1em] hover:bg-brand-ink transition-all flex items-center justify-center gap-3 shadow-xl">
  ...
  return (
    <div className="bg-brand-cream selection:bg-brand-primary selection:text-white">
      ...
      <section className="relative h-auto lg:min-h-[75vh] flex items-center pt-28 pb-12 md:pt-44 md:pb-20 overflow-hidden bg-brand-cream">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="flex flex-col lg:flex-row-reverse items-center gap-10 lg:gap-16">
            ...
            <div className="w-full lg:w-[50%] z-10 space-y-10 animate-fade-in text-center lg:text-left">
              <div className="space-y-6">
                <div className="flex items-center justify-center lg:justify-start gap-3">
                   <div className="w-10 h-[2px] bg-brand-primary"></div>
                   <span className="text-brand-primary font-black tracking-[0.4em] uppercase text-[10px]">Lumen Stone International</span>
                </div>
                ...
                <p className="text-brand-sub text-[17px] md:text-xl font-light leading-relaxed max-w-lg mx-auto lg:mx-0">
                  我們不只是留學代辦，更是教育的策展人。透過點石精選的紐西蘭校園，讓學習不再侷限於課本。
                </p>
              </div>
              <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-2">
                <Link to="/programs" className="px-10 py-4 bg-brand-primary text-white font-black rounded-xl hover:bg-brand-ink transition-all tracking-[0.15em] text-[13px] uppercase shadow-heavy flex items-center justify-center gap-4 group">
                  方案探索 <ArrowRight className="group-hover:translate-x-1 transition-transform" size={18} />
                </Link>
                <Link to="/guide" className="px-10 py-4 bg-white border-2 border-brand-primary text-brand-primary font-black rounded-xl hover:bg-brand-primary hover:text-white transition-all tracking-[0.15em] text-[13px] uppercase flex items-center justify-center shadow-sm">
                  選課指南
                </Link>
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
            <div className="w-full lg:w-[45%] order-1">
              <div className="relative">
                ...
                <div className="absolute -bottom-8 -right-4 md:-right-8 max-w-[200px] md:max-w-[300px] bg-white p-6 md:p-8 rounded-2xl shadow-heavy border border-brand-border animate-float">
                   <div className="flex gap-1 text-brand-accent mb-4">
                      {[1,2,3,4,5].map(i => <Star key={i} size={12} fill="currentColor"/>)}
                   </div>
                   <p className="text-[13px] md:text-base font-serif font-black text-brand-ink leading-relaxed">
                     「在紐西蘭，老師問的是：<br/>
            ...
            <div className="w-full lg:w-[55%] space-y-12 order-2">
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                   <div className="w-10 h-[1px] bg-brand-secondary"></div>
                   <span className="text-brand-secondary font-black tracking-[0.4em] uppercase text-[11px]">THE NEW ZEALAND WAY</span>
                </div>
                ...
                <p className="text-brand-cream/70 text-[17px] md:text-xl font-light leading-relaxed max-w-xl border-l-2 border-brand-accent/40 pl-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {[
                  { icon: <Award className="text-brand-secondary" size={24} />, title: "啟發式教學", desc: "引導孩子發現熱愛，而非填鴨知識。" },
                  { icon: <ShieldCheck className="text-brand-secondary" size={24} />, title: "安全友善", desc: "全球治安首選，讓學習更專注。" },
                  { icon: <GraduationCap className="text-brand-secondary" size={24} />, title: "頂尖學制", desc: "學歷通行全球，接軌一流名校。" },
                  { icon: <Sun className="text-brand-secondary" size={24} />, title: "大自然教室", desc: "在山海間學習生命力與韌性。" }
                ].map((item, idx) => (
                  <div key={idx} className="bg-white/5 p-7 rounded-2xl border border-white/5 space-y-4 hover:bg-white/10 transition-all">
                    <div className="flex items-center gap-4">
                    ...
                    <p className="text-[13px] text-brand-cream/50 leading-loose">{item.desc}</p>
                  </div>
                ))}
              </div>

              <div className="pt-2">
                 <Link to="/education" className="group relative inline-flex w-full md:w-auto items-center justify-center gap-4 px-10 py-4 bg-brand-secondary text-brand-primary rounded-xl font-black text-[13px] tracking-[0.1em] uppercase transition-all shadow-xl hover:bg-white">
                    探索紐西蘭學制 <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
                 </Link>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-6 lg:px-12">
           <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-16">
              <div className="space-y-4 text-center md:text-left">
                 <span className="text-brand-primary font-black tracking-[0.4em] uppercase text-[11px] block">Insights & Updates</span>
              ...
              {news.map((item, idx) => (
                ...
                >
                   <div className="relative aspect-[16/10] rounded-xl overflow-hidden mb-8">
                      ...
                      <div className="absolute top-5 right-5 bg-brand-primary text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md">
                         {item.category}
                      </div>
                   </div>
                   <div className="space-y-5">
                      <div className="flex items-center gap-5 text-brand-accent font-black text-[11px] uppercase tracking-[0.2em]">
                         <span>{item.date}</span>
                         <div className="h-[1px] flex-grow bg-brand-border/60"></div>
                      </div>
                      <h3 className="text-2xl font-serif font-black text-brand-ink group-hover:text-brand-primary transition-colors leading-tight line-clamp-2">
                         {item.title}
                      </h3>
                      <p className="text-brand-sub text-[16px] font-light leading-relaxed line-clamp-2 opacity-80">
                         {item.summary}
                      </p>
                      <div className="pt-4 flex items-center gap-3 text-brand-primary font-black text-[11px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                         閱讀完整內容 <ArrowRight size={16} />
                      </div>
                   </div>
                </div>
              ))}
      ...
      <section className="py-24 md:py-40 bg-white">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="max-w-4xl mx-auto text-center mb-20 md:mb-32">
             <span className="text-brand-primary font-black tracking-[0.5em] uppercase text-[11px] block mb-6">EDUCATION CURATION</span>
             <h2 className="text-5xl md:text-7xl font-serif font-black text-brand-ink tracking-tighter">為您規劃最理想的起點</h2>
             <p className="mt-8 text-brand-sub text-[18px] md:text-xl font-light max-w-2xl mx-auto">
            ...
            {[
              ...
              <div key={idx} className="group bg-brand-cream rounded-3xl overflow-hidden shadow-zen border border-brand-border flex flex-col hover:border-brand-primary/40 transition-all duration-500 hover:-translate-y-2">
                ...
                <div className="p-10 md:p-12 flex-grow flex flex-col space-y-8">
                  <div>
                    <h3 className="text-3xl font-serif font-black text-brand-ink mb-3">{item.title}</h3>
                    <p className="text-[11px] text-brand-primary font-black uppercase tracking-widest">{item.type}</p>
                  </div>
                  <div className="space-y-6">
                    <div className="flex items-start gap-4">
                      <Clock className="w-5 h-5 text-brand-primary mt-1 shrink-0" />
                      <div>
                        <p className="text-[10px] font-black text-brand-ink/30 uppercase tracking-widest mb-1">Duration</p>
                        <p className="text-lg font-bold text-brand-ink">{item.duration}</p>
                      </div>
                    </div>
                    <div className="flex flex-col gap-4">
                       <p className="text-[16px] text-brand-ink/80 leading-relaxed font-bold">{item.value}</p>
                       <ul className="space-y-3">
                         {item.bullets.map((b, bi) => (
                           <li key={bi} className="flex items-start gap-3 text-[14px] text-brand-sub font-light">
                             <Check size={16} className="text-brand-primary shrink-0 mt-1" />
                             {b}
                           </li>
                         ))}
                       </ul>
                  ...
                  <div className="pt-8 border-t border-brand-border flex items-center justify-between mt-auto">
                    <Link to="/programs" className="flex items-center gap-2 text-brand-primary font-black text-[13px] uppercase tracking-widest group-hover:gap-4 transition-all">
                      瀏覽完整細節 <ArrowUpRight size={18} />
                    </Link>
                  </div>
                </div>
              </div>
      ...
      <section className="py-24 md:py-40 bg-brand-primary text-white relative">
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          <div className="text-center mb-32">
             <h2 className="text-5xl md:text-7xl font-serif font-black mb-6">服務流程</h2>
             <span className="text-brand-secondary tracking-[0.5em] uppercase text-[11px] font-black opacity-60">SIMPLE 5 STEPS TO THE WORLD</span>
          </div>

          <div className="max-w-5xl mx-auto relative">
            <div className="absolute top-[40px] left-[10%] w-[80%] h-[1px] bg-white/10 hidden lg:block z-0"></div>
            <div className="absolute left-[39px] top-12 bottom-12 w-[1px] bg-white/10 lg:hidden border-l-2 border-brand-secondary/30 border-dashed"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-16 lg:gap-12">
              {[
                ...
                <div key={i} className="flex flex-row lg:flex-col items-start lg:items-center text-left lg:text-center group gap-8 lg:gap-0">
                  <div className="relative shrink-0 mb-0 lg:mb-12">
                    ...
                    <div className="absolute -top-1 -right-1 w-7 h-7 bg-brand-accent text-white rounded-full flex items-center justify-center text-[11px] font-black border-2 border-brand-primary">
                      {step.id}
                    </div>
                  </div>
                  <div className="space-y-4 lg:pt-0 pt-3">
                    <h4 className="text-2xl font-serif font-bold text-white group-hover:text-brand-secondary transition-colors">{step.title}</h4>
                    <p className="text-[15px] text-white/50 leading-relaxed max-w-[180px] lg:mx-auto font-light">{step.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-32 text-center">
             <Link to="/booking" className="inline-flex items-center gap-5 px-12 py-5 bg-brand-secondary text-brand-primary rounded-xl font-black text-[13px] tracking-[0.2em] uppercase hover:bg-white transition-all shadow-heavy active:scale-95">
      ...
      {/* FINAL CTA - Enhanced with Background Image and Tint */}
      <section className="relative py-48 md:py-64 flex items-center justify-center overflow-hidden">
         {/* Background with inspirational imagery and overlay */}
         <div className="absolute inset-0 z-0">
            ...
            />
            {/* Dark tinted mask with subtle gradient */}
            <div className="absolute inset-0 bg-brand-ink/75 backdrop-blur-[1px]"></div>
            <div className="absolute inset-0 bg-gradient-to-t from-brand-ink via-transparent to-brand-ink/40"></div>
         </div>

         <div className="container mx-auto px-6 text-center relative z-10">
            <div className="max-w-4xl mx-auto space-y-12">
               <span className="text-brand-secondary font-black tracking-[0.6em] uppercase text-[12px] block opacity-80">BEYOND BOUNDARIES</span>
               <h2 className="text-6xl md:text-9xl font-serif font-black leading-none tracking-tighter text-white drop-shadow-2xl">
                 Explore<br/><span className="text-brand-secondary italic">Your Future</span>
               </h2>
               <div className="flex justify-center pt-8">
                 <Link to="/booking" className="inline-flex items-center gap-6 px-14 py-6 bg-brand-secondary text-brand-primary font-black rounded-xl transition-all text-[14px] tracking-[0.3em] uppercase hover:bg-white shadow-2xl hover:scale-105 active:scale-95">
                    預約一對一諮詢 <ArrowRight size={28} />
                 </Link>
               </div>
               <p className="text-white/40 text-base font-light mt-16 tracking-widest">專業顧問將於 24 小時內與您聯繫</p>
            </div>
         </div>
      </section>
    </div>
